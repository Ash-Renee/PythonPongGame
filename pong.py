...
delay = 30 #makes the game go faster or slower (bigger number = slower)
...
ball_y_pos = HEIGHT / 2
ball_width = 8
ball_x_vel = -10 #the negative makes it go left vice right
ball_y_vel = 0
ball_color = (randint(0,200), randint(0,200), randint(0,200))

screen = pygame.display.set_mode((WIDTH, HEIGHT))

#drawing objects
def draw_objects(): #the game and the relative position of items
...
def apply_player_movement(): #ensures that we don't move paddles out of bounds
    global p1_y_pos
    global p2_y_pos
...
def apply_ball_movement(): #ball physics logic
    global ball_x_pos
    global ball_x_vel
    global ball_y_pos
    global ball_y_vel
    ...
    elif ball_x_pos + ball_x_vel < 0: # player 2 scores logic
        p2_score += 1
        ball_x_pos = WIDTH / 2
        ball_y_pos = HEIGHT / 2
        ball_x_vel = 10
    ...
    elif ball_x_pos + ball_x_vel > HEIGHT: #player 1 score logic
        p1_score += 1
        ball_x_pos = WIDTH / 2
        ball_y_pos = HEIGHT / 2
        ball_x_vel = -10
...
pygame.display.set_caption("LilyPong v0.1 Alpha")
screen.fill(BLACK)
pygame.display.flip()
